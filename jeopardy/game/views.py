...
def index(request):
	air_date_list = Questions.objects.all().values('air_date').order_by('air_date').distinct()
	context = RequestContext(request, {'air_date_list': air_date_list,})
	return render(request, 'game/index.html', context)


def date(request, air_date):
	categories_list = Questions.objects.all().values('category').filter(air_date=air_date, current_round='Jeopardy!').distinct()
